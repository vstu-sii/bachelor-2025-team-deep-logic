...
|-----------------|----------------|------------------------------------|-------------------|
| Backend API     | RabbitMQ       | Постановка и получение задач       | AMQP, JSON        |
| Backend API     | VLM (Ollama)   | Распознавание изображения          | HTTP/gRPC, JSON   |
| Backend API     | LLM API        | Генерация рецептов                 | HTTPS, JSON       |
| Backend API     | SQLite         | Хранение пользователей и истории   | SQL               |

### Фактические решения

- Model Manager встроен в Backend API.  
- Основная база данных — SQLite. 
- Уведомления отсутствуют в текущем прототипе.  
- Мониторинг и метрики (Grafana/Prometheus) планируются в следующих спринтах.  
