...
- Время генерации списка рецептов ≤ 30 секунд.  
...
### 4.6 Безопасность
- В учебном прототипе реализована упрощённая проверка учётных данных (сравнение строк в базе).  
- Пароли пока хранятся в открытом виде, что допустимо только для демонстрационных целей.    
...
- Используется модель **C4** для описания архитектуры:  
  ...
  - Container Diagram — веб‑приложение, Backend API, база данных.  
...
- Принцип **Separation of Concerns**: каждая часть системы отвечает за свою задачу.  
  API обрабатывает запросы пользователей, бизнес‑логика решает правила работы приложения, интеграции связывают нас с внешними сервисами, а база данных хранит информацию.  
  Это упрощает поддержку и развитие системы.
- Принцип **Fail‑safe defaults**: если внешний сервис не отвечает или произошёл сбой, система не ломается.  
  Вместо ошибки пользователю возвращается безопасный и понятный ответ (например: «рецепт временно недоступен, попробуйте позже»).  
  Это делает работу приложения стабильной даже при проблемах с интеграциями.


---

## 7. Ограничения и допущения
- VLM‑модуль и LLM API предоставляются как отедельные сервисы (не разрабатываются в рамках проекта).  
- Система ориентирована на работу в облачной инфраструктуре.  
- Поддерживаются только современные браузеры (Chrome, Firefox, Edge, Safari).
